...
AppDeckItem {
    ...
    onDragStarted: function() {
        ...
        AppDeckController.onDragStart(String(desktopFile || name || index),
    ...
    onDragMoved: function(deltaX) {
        ...
        AppDeckController.onDragMove(deltaX, dockRoot ? dockRoot.hostName : "")
    ...
    onDragFinished: function(deltaX) {
        ...
        AppDeckController.onDragEnd(dockRoot ? dockRoot.hostName : "")
        dockRoot.finishReorderDrag(deltaX)
    }

    // Returns true if the given compositor appId matches this appdeck entry.
    // Uses the same identity tokens as AppDeck._hasIdToken so the logic is consistent.
    ...
    TapHandler {
        ...
        onTapped: {
            AppDeckController.onContextMenu(String(desktopFile || name || ""),
                                         dockRoot ? dockRoot.hostName : "")
    ...
    Menu {
        ...
        onAboutToShow: {
            if (typeof AppModel !== "undefined" && AppModel
                ...
                quickRows = AppModel.slmQuickActionsForEntry("appdeck", {
                ...
                quickRows = AppModel.slmQuickActions("appdeck") || []
            } else {
                quickRows = []
            }
        ...
        MenuItem {
            ...
            onTriggered: {
                if (desktopFile && desktopFile.length > 0) {
                    AppDeckModel.addDesktopEntry(desktopFile)
                }
            }
        }
        MenuItem {
            ...
            onTriggered: {
                if (desktopFile && desktopFile.length > 0) {
                    AppDeckModel.removeDesktopEntry(desktopFile)
                }
            }
        }
        MenuItem {
            ...
            onTriggered: AppDeckModel.movePinnedEntry(index, index - 1)
        }
        MenuItem {
            text: "Move Right"
            visible: pinnedEntry
