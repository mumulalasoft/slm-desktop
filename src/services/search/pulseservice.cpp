...
QString emptyQueryCachePath()
{
    ...
    QDir dir(base);
    dir.mkpath(QStringLiteral("pulse"));
    return dir.filePath(QStringLiteral("pulse/empty_query_cache.json"));
}
...
QVariantList queryCapabilitySearchActions(const QString &needle, int limit)
{
    ...
    QVariantMap context{
        ...
        {QStringLiteral("source_app"), QStringLiteral("org.slm.pulse")},
...
PulseService::PulseService(QObject *parent)
    : QObject(parent)
...
bool PulseService::ready() const
{
    return true;
}

QVariantList PulseService::query(const QString &text, const QVariantMap &options, int limit)
{
    ...
    if (iface.isValid()) {
        ...
        if (!queryOpts.contains(QStringLiteral("includeApps"))) {
            queryOpts.insert(QStringLiteral("includeApps"), true);
        }
        if (!queryOpts.contains(QStringLiteral("includeRecent"))) {
            queryOpts.insert(QStringLiteral("includeRecent"), true);
        }
        if (!queryOpts.contains(QStringLiteral("includeClipboard"))) {
            queryOpts.insert(QStringLiteral("includeClipboard"), true);
        }
        if (!queryOpts.contains(QStringLiteral("includeTracker"))) {
            queryOpts.insert(QStringLiteral("includeTracker"), true);
        }
        if (!queryOpts.contains(QStringLiteral("includeActions"))) {
            queryOpts.insert(QStringLiteral("includeActions"), true);
        }
        if (!queryOpts.contains(QStringLiteral("includeSettings"))) {
            queryOpts.insert(QStringLiteral("includeSettings"), true);
        }
        if (!queryOpts.contains(QStringLiteral("includePreview"))) {
            queryOpts.insert(QStringLiteral("includePreview"), true);
        }
        queryOpts.insert(QStringLiteral("scope"), QStringLiteral("pulse"));
        queryOpts.insert(QStringLiteral("source_app"), QStringLiteral("org.slm.pulse"));
    ...
    const auto materializeActionRows = [&](const QVariantList &rows, const QString &tag) -> QVariantList {
        ...
        if (!out.isEmpty()) {
            ...
            qInfo().noquote() << "[slm-search] pulse-fallback-return source=" << tag
                              << "query=" << needle
                              << "count=" << out.size()
        ...
        if (invalidRows > 0) {
            qInfo().noquote() << "[slm-search] pulse-fallback-invalid source=" << tag
                              << "query=" << needle
    ...
    if (!actionRows.isEmpty()) {
        qInfo().noquote() << "[slm-search] pulse-fallback query=" << needle
    ...
    if (isEmptyQuery) {
        ...
        if (!localFallback.isEmpty()) {
            ...
            qInfo().noquote() << "[slm-search] pulse-empty-local-fallback count=" << localFallback.size();
...
bool PulseService::activateResult(const QString &id, const QVariantMap &activateData)
{
    ...
    if (rid.startsWith(QStringLiteral("slm-action:"))) {
        ...
        if (!invokeCtx.contains(QStringLiteral("scope"))) {
            invokeCtx.insert(QStringLiteral("scope"), QStringLiteral("pulse"));
        }
        
        QVariantMap reply = framework.invokeAction(actionId, invokeCtx);
        if (reply.value(QStringLiteral("ok")).toBool()) {
            qInfo().noquote() << "[slm-search] pulse-fallback-invoke action=" << actionId << "success";
            return true;
        }
        qWarning().noquote() << "[slm-search] pulse-fallback-invoke action=" << actionId << "failed" 
...
QVariantList PulseService::loadEmptyQueryCache() const
{
...
void PulseService::storeEmptyQueryCache(const QVariantList &rows)
{
    if (rows.isEmpty()) {
        return;
    }
...
QVariantList PulseService::localPopularAppFallback(int limit) const
{
    QVariantList out;
    if (limit <= 0) {
        return out;
...
QVariantMap PulseService::previewResult(const QString &id)
{
    const QString rid = id.trimmed();
    if (rid.isEmpty()) {
...
QVariantMap PulseService::resolveClipboardResult(const QString &id,
...
QVariantMap PulseService::configureTrackerPreset(const QVariantMap &preset)
{
...
QVariantMap PulseService::resetTrackerPreset()
{
...
QVariantMap PulseService::trackerPresetStatus()
{
    QDBusInterface iface(QString::fromLatin1(kSearchService),
                         QString::fromLatin1(kSearchPath),
                         QString::fromLatin1(kSearchIface),
...
QVariantList PulseService::searchProfiles()
{
    QDBusInterface iface(QString::fromLatin1(kSearchService),
                         QString::fromLatin1(kSearchPath),
                         QString::fromLatin1(kSearchIface),
...
QString PulseService::activeSearchProfile()
{
    QDBusInterface iface(QString::fromLatin1(kSearchService),
                         QString::fromLatin1(kSearchPath),
                         QString::fromLatin1(kSearchIface),
...
QVariantMap PulseService::activeSearchProfileMeta()
{
    QDBusInterface iface(QString::fromLatin1(kSearchService),
                         QString::fromLatin1(kSearchPath),
                         QString::fromLatin1(kSearchIface),
...
bool PulseService::setActiveSearchProfile(const QString &profileId)
{
...
QVariantMap PulseService::telemetryMeta()
{
...
QVariantList PulseService::activationTelemetry(int limit)
{
...
bool PulseService::resetActivationTelemetry()
{
...
void PulseService::handleSearchProfileChanged(const QString &profileId)
{
    emit searchProfileChanged(profileId);
}

void PulseService::setupSecurity()
{
    if (!m_permissionStore.open()) {
        qWarning() << "[pulse] failed to open permission store";
    }
    ...
    m_securityGuard.registerMethodCapability(QStringLiteral("org.slm.Pulse"), QStringLiteral("Query"), Slm::Permissions::Capability::SearchQueryApps);
    m_securityGuard.registerMethodCapability(QStringLiteral("org.slm.Pulse"), QStringLiteral("ActivateResult"), Slm::Permissions::Capability::QuickActionInvoke);
    m_securityGuard.registerMethodCapability(QStringLiteral("org.slm.Pulse"), QStringLiteral("ResolveClipboardResult"), Slm::Permissions::Capability::SearchResolveClipboardResult);
    m_securityGuard.registerMethodCapability(QStringLiteral("org.slm.Pulse"), QStringLiteral("ConfigureTrackerPreset"), Slm::Permissions::Capability::SearchConfigure);
    m_securityGuard.registerMethodCapability(QStringLiteral("org.slm.Pulse"), QStringLiteral("ResetTrackerPreset"), Slm::Permissions::Capability::SearchConfigure);
    m_securityGuard.registerMethodCapability(QStringLiteral("org.slm.Pulse"), QStringLiteral("SetActiveSearchProfile"), Slm::Permissions::Capability::SearchConfigure);
    m_securityGuard.registerMethodCapability(QStringLiteral("org.slm.Pulse"), QStringLiteral("GetTelemetryMeta"), Slm::Permissions::Capability::SearchReadTelemetry);
    m_securityGuard.registerMethodCapability(QStringLiteral("org.slm.Pulse"), QStringLiteral("GetActivationTelemetry"), Slm::Permissions::Capability::SearchReadTelemetry);
    m_securityGuard.registerMethodCapability(QStringLiteral("org.slm.Pulse"), QStringLiteral("ResetActivationTelemetry"), Slm::Permissions::Capability::SearchReadTelemetry);
}

bool PulseService::checkPermission(Slm::Permissions::Capability capability, const QString &methodName)
{
    if (!calledFromDBus()) {
        return true;
    }
