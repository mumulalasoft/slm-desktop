...
{
    return AppDeckIdentity::normalizeToken(value);
}
...
{
    return AppDeckIdentity::isGenericWrapperToken(tokenRaw);
}

bool isWrapperLikeToken(const QString &tokenRaw)
{
    return AppDeckIdentity::isWrapperLikeToken(tokenRaw);
}

bool isWeakIdentityToken(const QString &tokenRaw)
{
    return AppDeckIdentity::isWeakIdentityToken(tokenRaw);
}

bool isStrongIdentityToken(const QString &tokenRaw)
{
    return AppDeckIdentity::isStrongIdentityToken(tokenRaw);
}
...
{
    return AppDeckIdentity::filterLaunchHintTokens(tokens);
}
...
AppDeckModel::AppDeckModel(QObject *parent)
    : QAbstractListModel(parent)
    , m_localSettings()
{
    m_verboseLogging = m_localSettings.value(QStringLiteral("debug/verboseLogging"), false).toBool();
    ensureDockDirectory();
    QTimer::singleShot(0, this, &AppDeckModel::refresh);

    m_runningWatchDebounce.setSingleShot(true);
    m_runningWatchDebounce.setInterval(120);
    connect(&m_runningWatchDebounce, &QTimer::timeout, this, &AppDeckModel::refreshRunningStates);
    connect(&m_procWatcher, &QFileSystemWatcher::directoryChanged, this, &AppDeckModel::scheduleRunningRefresh);
    setupProcessWatcher();

    m_runningPollTimer.setInterval(700);
    m_runningPollTimer.setSingleShot(false);
    connect(&m_runningPollTimer, &QTimer::timeout, this, &AppDeckModel::refreshRunningStates);
    m_runningPollTimer.start();
}

int AppDeckModel::rowCount(const QModelIndex &parent) const
{
    if (parent.isValid()) {
        return 0;
    }
...
QVariant AppDeckModel::data(const QModelIndex &index, int role) const
{
...
QHash<int, QByteArray> AppDeckModel::roleNames() const
{
...
QString AppDeckModel::dockDirectoryPath() const
{
    return QDir::home().filePath(QStringLiteral(".AppDeck"));
}

QString AppDeckModel::orderFilePath() const
{
    return QDir(dockDirectoryPath()).filePath(QStringLiteral(".order"));
}

bool AppDeckModel::ensureDockDirectory()
{
...
QStringList AppDeckModel::loadDockOrder() const
{
    QStringList order;
...
void AppDeckModel::saveDockOrder(const QVector<DockEntry> &entries) const
{
    QFile file(orderFilePath());
...
bool AppDeckModel::addDesktopEntry(const QString &sourceDesktopFilePath)
{
...
bool AppDeckModel::addDesktopEntryAt(const QString &sourceDesktopFilePath, int pinnedPosition)
{
    const QFileInfo srcInfo(sourceDesktopFilePath);
    if (!srcInfo.exists() || !srcInfo.isFile() || srcInfo.suffix().toLower() != QStringLiteral("desktop")) {
        return false;
...
bool AppDeckModel::removeDesktopEntry(const QString &desktopFilePath)
{
...
bool AppDeckModel::launchDesktopEntry(const QString &desktopFilePath, const QString &executable)
{
...
bool AppDeckModel::activateExistingWindow(const QString &desktopFilePath, const QString &executable,
                                       const QString &displayName)
{
...
bool AppDeckModel::activateOrLaunch(const QString &desktopFilePath, const QString &executable,
                                 const QString &displayName)
{
...
void AppDeckModel::noteLaunchedEntry(const QString &desktopFilePath, const QString &executable,
                                  ...
                                  const QString &iconSource)
{
    ...
    for (int i = 0; i < m_entries.size(); ++i) {
        ...
        if (sameDesktop || sameExec || sameName) {
            ...
            QTimer::singleShot(300, this, &AppDeckModel::refreshRunningStates);
            QTimer::singleShot(1500, this, &AppDeckModel::refreshRunningStates);
            return;
        }
    }
...
bool AppDeckModel::focusExistingWindow(const QString &desktopFilePath, const QString &executable,
...
bool AppDeckModel::movePinnedEntry(int fromModelIndex, int toModelIndex)
{
...
bool AppDeckModel::isEntryRunning(const DockEntry &entry, const QSet<QString> &runningExecutables) const
{
...
bool AppDeckModel::hasWindowForEntry(const DockEntry &entry, const QStringList &wmctrlLines) const
{
...
void AppDeckModel::setupProcessWatcher()
{
...
void AppDeckModel::scheduleRunningRefresh()
{
    m_runningWatchDebounce.start();
}

void AppDeckModel::refreshRunningStates()
{
    m_verboseLogging = m_localSettings.value(QStringLiteral("debug/verboseLogging"), false).toBool();
    setupProcessWatcher();
        ...
        if (!wmctrlPath.isEmpty()) {
            ...
            connect(w, &QFutureWatcher<QStringList>::finished, this, [this, w]() {
                ...
                if (fresh != m_wmctrlCache) {
                    ...
                    QTimer::singleShot(0, this, &AppDeckModel::refreshRunningStates);
                }
            });
